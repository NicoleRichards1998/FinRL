...
class DayTradingEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        if self.turbulence_bool[self.day] == 0:
            #min_action = int(self.max_stock * self.min_stock_rate)  # stock_cd
            min_action = self.max_stock * self.min_stock_rate # stock_cd
            for index in np.where(actions < -min_action)[0]:  # sell_index:
                if price[index] > 0:  # Sell only if current asset is > 0
                    sell_num_shares = min(self.stocks[index], -actions[index])
            ...
            for index in np.where(actions > min_action)[0]:  # buy_index:
                if (
                    ...
                    buy_num_shares = min(self.amount // (price[index]*1000), actions[index])
            ...
            print("selling all stock because of turbulence")
            self.amount += (self.stocks * price).sum() * (1 - self.sell_cost_pct)
            self.stocks[:] = 0
            self.stocks_cool_down[:] = 0
...
class ChangedStockTradingEnv(gym.Env):
    def __init__(
        self,
        config,
        initial_account=1e6,
...
class EndRewardStockTradingEnv(gym.Env):
    def __init__(
        self,
        config,
        initial_account=1e6,
